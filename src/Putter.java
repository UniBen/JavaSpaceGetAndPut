import net.jini.space.*;
import net.jini.core.lease.*;
import java.awt.*;
import javax.swing.*;

public class Putter extends JFrame {
	private static int ONE_MINUTE = 1000 * 60;  //one minute = 1000 miliseconds, multiplied by 60
	
	private JavaSpace space;

	private JPanel jPanel1, jPanel2, jPanel3, jPanel4, jPanel5;
	private JLabel textLabel;
	private JTextField inString, inCount;
	private JButton putButton;
	private JButton putManyButton;


	public Putter() {
		space = SpaceUtils.getSpace();
		if (space == null){
			System.err.println("Failed to find the javaspace");
			System.exit(1);
		}

		initComponents ();
		pack ();
	}

	private void initComponents () {
		setTitle ("JavaSpaces Putter");
		addWindowListener (new java.awt.event.WindowAdapter () {
			public void windowClosing (java.awt.event.WindowEvent evt) {
				exitForm (evt);
			}
		}   );

		Container cp = getContentPane();
		cp.setLayout (new BorderLayout ());

		jPanel1 = new JPanel ();
		jPanel1.setLayout (new FlowLayout ());

		textLabel = new JLabel ();
		textLabel.setText ("Value to insert ");
		jPanel1.add (textLabel);

		inString = new JTextField (12);
		inString.setText ("");

		jPanel1.add (inString);

		cp.add (jPanel1, "North");

		jPanel2 = new JPanel ();
		jPanel2.setLayout (new GridLayout (2, 1));

		jPanel4 = new JPanel ();
		jPanel4.setLayout (new FlowLayout ());

		inCount = new JTextField (5);
		inCount.setText ("");
		jPanel4.add (inCount);

		jPanel2.add (jPanel4);

		jPanel5 = new JPanel ();
		jPanel5.setLayout (new FlowLayout ());

		putManyButton = new JButton ();
		putManyButton.setText ("Insert  Lots");
		putManyButton.addActionListener (new java.awt.event.ActionListener () {
			public void actionPerformed (java.awt.event.ActionEvent evt) {
				writeMany (evt);
			}
		}  );

		jPanel5.add (putManyButton);
		jPanel2.add (jPanel5);

		cp.add (jPanel2, "East");

		jPanel3 = new JPanel ();
		jPanel3.setLayout (new FlowLayout ());

		putButton = new JButton ();
		putButton.setText ("Insert One");
		putButton.addActionListener (new java.awt.event.ActionListener () {
			public void actionPerformed (java.awt.event.ActionEvent evt) {
				writeSobj (evt);
			}
		}  );

		jPanel3.add (putButton);
		cp.add (jPanel3, "West");
	}

	private void writeMany (java.awt.event.ActionEvent evt) {
		try {
			Sobj str = new Sobj(inString.getText());
			int count = (new Integer(inCount.getText())).intValue();
			if (count > 20)
				JOptionPane.showMessageDialog(this,
						"Grow Up!  Count must be 1 to 5",
						"Error",
						JOptionPane.ERROR_MESSAGE,
						null);
			else if (count > 5)
				JOptionPane.showMessageDialog(this,
						"Count must be 1 to 5",
						"Error",
						JOptionPane.ERROR_MESSAGE,
						null);
			else {
				for ( int i= 0; i < count ; i++ ) {
					space.write(str, null, ONE_MINUTE);
				}
			}
		} catch ( Exception e) {
			e.printStackTrace();
		}
	}

	private void writeSobj (java.awt.event.ActionEvent evt) {
		try {
			Sobj str = new Sobj(inString.getText());
			space.write( str, null, ONE_MINUTE);
		} catch ( Exception e) {
			e.printStackTrace();
		}
	}

	private void exitForm(java.awt.event.WindowEvent evt) {
		System.exit (0);
	}


	public static void main(String[] args) {
		new Putter().setVisible(true);
	}    
}
